...
# --- Contournements demontres en revue sur CREDENTIAL_ARGV_MARKERS ---------
# La revue a fabrique 3 manifestes (guard_secrets.check() execute dessus, pas
# de mutation du rendu reel) qui passaient au VERT alors qu'ils vehiculent un
# credential en argv -- exactement l'invariant que cette garde a ete ajoutee
# pour fermer. Chaque test ci-dessous est rouge AVANT le correctif (preuve
# capturee dans infra-debt-report.md), vert APRES.

def test_bypass1_dash_a_flag_merged_without_space_is_caught():
    # `-a"$VAR"` (accole, SANS espace) est un idiome shell valide (meme forme
    # que `mysql -p$PASSWORD`) qui fusionne le flag et sa valeur en un seul
    # token -- l'ancien marqueur substring `"-a "` (espace final littéral)
    # ne matche jamais cette forme puisque la sous-chaine "-a " (avec espace)
    # n'apparait alors nulle part dans l'argv.
    doc = """
apiVersion: apps/v1
kind: Deployment
metadata:
...
"""
    problems = guard_secrets.check(doc)
    assert any("evil/evil" in p for p in problems), problems


def test_bypass2_inline_credential_in_argv_connection_string_is_caught():
    # INLINE_CRED_RE (scheme://user:pass@host) n'etait applique qu'aux
    # `env[].value` -- jamais au texte de `command`/`args`, alors qu'une
    # connection string passee en argv (`psql postgresql://app:hunter2@host`)
    # est exactement le meme risque CWE-214.
    doc = """
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  template:
    spec:
      containers:
        - name: evil
          command: ["sh", "-c", "psql postgresql://app:hunter2@host -c 'select 1'"]
"""
    problems = guard_secrets.check(doc)
    assert any("evil/evil" in p and "credential inline" in p for p in problems), problems


def test_bypass3_dash_dash_pass_flag_is_caught():
    # `--pass` (forme courte de `--password` utilisee par de nombreux outils
    # generiques) n'etait pas dans CREDENTIAL_ARGV_MARKERS -- seul
    # `--password` y figurait.
    doc = """
apiVersion: apps/v1
kind: Deployment
...
spec:
  template:
    spec:
      containers:
        - name: evil
          command: ["sh", "-c", "mytool --pass \\"$SECRET\\""]
"""
    problems = guard_secrets.check(doc)
    assert any("evil/evil" in p and "--pass" in p for p in problems), problems


def test_dash_a_bare_or_positional_flag_is_not_a_false_positive():
    # `-a` est aussi un flag legitime TRES courant sans valeur credential qui
    # le suit immediatement : `tar -a` (auto-compression, souvent suivi d'un
    # AUTRE flag), `cp -a` (archive, suivi d'un chemin positionnel), `ls -a`
    # (isole). Le motif corrige exige qu'un guillemet/`$` (forme quasi
    # systematique d'une valeur credential interpolee) suive IMMEDIATEMENT le
    # flag (accole ou apres un espace) -- un chemin/flag ordinaire ne doit
    # JAMAIS declencher la garde.
    doc = """
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  template:
    spec:
      containers:
        - name: fine
          command: ["sh", "-c", "tar -a -czf /backup/data.tar.gz /data && cp -a /data /backup/data-copy && ls -a /backup"]
"""
    assert guard_secrets.check(doc) == []
