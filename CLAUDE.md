...
1. **JAMAIS de build prod manuel** — build local Docker Desktop pour dev/test uniquement
   ...
   **Local = build** : backend ET frontend se **buildent localement** dans le groupe `facil_framework`
   (services par défaut, `build:`, zéro registre/login — comme `backend-1`). **Prod/VPS = pull** : la CI
   (`.github/workflows/release-images.yml`) build+push `ghcr.io/<owner>/facil-{backend,web}` (push `main`/`develop`),
   le serveur **pull** ces images (pas de build sur le serveur). En local, **option** de pull au lieu de builder
   (ex. si le build Next OOM) = **`tools/refresh-local.sh`** (override `deploy/compose.images.yml`). Jamais de build prod manuel.
